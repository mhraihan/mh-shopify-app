...
@section('title', 'Product - Edit')
 @php
  ...
  @endphp
@section('content')
  @include("_partials.breadcrumbs",['parent' => "Product",'child' => 'Edit Product'])
  <div class="row">
    <div class="col-12">
      <div class="card mb-4">
        <div class="card-body">
          <form method="post" action="{{ route('products.update',['product' => $id]) }}">
            ...
            <div class="mb-3">
              ...
              <input type="text" class="form-control" id="basic-default-title" name="title" required
                     value="{{ old('title',$product['title']) }}" placeholder="Collection title"/>
              @error('title')
            ...
            <div class="mb-3">
              ...
              <textarea id="basic-default-message" class="form-control" required name="description"
                        placeholder="Collection Description">{{ old('description',$product['descriptionHtml']) }}</textarea>
              @error('description')
              <span class="text-danger">{{ $message }}</span>
              @enderror
            </div>
